...
export function reconcileFinalDemoSequences1to4(
  ...
  if (bySeq.size !== input.messages.length) {
    throw new FinalDemoError("Duplicate sequences in Mirror window", "DUPLICATE_SEQUENCE");
  }
  for (let s = 1; s <= 4; s++) {
    if (!bySeq.has(s)) {
  ...
  if (input.messages.some((m) => m.sequence > 4 || m.sequence < 1)) {
    // sequences beyond 4 not yet allowed before payment
    const extra = input.messages.filter((m) => m.sequence > 4);
    if (extra.length > 0) {
      throw new FinalDemoError(
        "Unexpected messages beyond sequence 4 before payment",
        "UNEXPECTED_SEQUENCE",
      );
    }
  }
